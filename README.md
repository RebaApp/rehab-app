...
- **Статьи** - Полезные материалы о реабилитации и здоровье с рубриками
...
### Админ панель и редактор статей
- **Админ панель** - Полноценная система управления контентом
- **RichTextEditor** - Редактор статей с форматированием (жирный, курсив, заголовки, списки, цитаты)
- **Предпросмотр статей** - Просмотр как будет выглядеть статья перед публикацией
- **Рубрики статей** - Организация контента по темам (Наркомания, Алкоголизм, Лудомания, Табакокурение, Восстановление и др.)
- **Профессиональное авторство** - Разделение на ФИО автора и регалии
- **Типы статей** - Медиа-статьи для бренда и интеграционные для привлечения клиентов
- **Создание центров** - Добавление новых реабилитационных центров через админ панель
...
│   │       ├── AdminModal.tsx       # Модальное окно админ панели
│   │       ├── CreateArticleModal.tsx # Форма создания статей
│   │       ├── CreateCenterModal.tsx  # Форма создания центров
│   │       ├── RichTextEditor.tsx  # Редактор с форматированием
│   │       └── FormattedText.tsx   # Отображение форматированного текста
...
### RichTextEditor для создания статей
```typescript
<RichTextEditor
  value={articleBody}
  onChangeText={setArticleBody}
  placeholder="Введите текст статьи с форматированием..."
  style={styles.editor}
/>
```
...
const newArticle = addArticle({
  ...
  body: "**Жирный текст** и *курсив*",
  authorName: "Иванов И.И.",
  authorCredentials: "Врач центра",
  rubric: "Алкоголизм",
  articleType: "media"
});
```
...
- **Вадим Давыдов** - Основатель проекта, главный разработчик
  - Telegram: [@vadimsao](https://t.me/vadimsao)
  - Специализация: React Native, TypeScript, Full-stack разработка
